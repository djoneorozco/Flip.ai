...
// ============================================================
// ✅ /api/enhance — Real DALL·E createEdit() with real uploaded PNG
// ============================================================
app.post('/api/enhance', upload.single('image'), async (req, res) => {
  ...
  if (!req.file) {
    ...
    return res.status(400).json({ error: "Image is required and must be a PNG" });
  }
  ...
  if (investment < 20000) {
    tier = "Tier 1 — Basic";
    stylePrompt = `
Photo of the house with boarded-up windows.
Replace only the boarded-up windows with clear glass windows matching the house’s style.
Do not change paint, yard, roof, door, or anything else. Keep realistic look.
    `.trim();
  } else if (investment < 50000) {
    tier = "Tier 2 — Moderate";
    stylePrompt = `
Photo of the house with boarded-up windows and old paint.
Replace windows with clear modern windows, repaint the exterior with fresh coat matching original color, add simple landscaping (fresh grass, trimmed bushes). Keep realistic, no major redesign.
    ...
    tier = "Tier 3 — Premium";
    stylePrompt = `
Photo of the house ready for a high-end flip.
Replace old windows with luxury windows, repaint exterior with modern designer color, upgrade front door with modern style, add upscale landscaping (plants, pathway stones, trimmed lawn).
Keep realistic look for a premium flip.
  ...
  try {
    ...
    res.json({
      enhancedImageUrl,
      tierUsed: tier,
      description: "Generated by Flip.ai with real DALL·E edit & tiered prompt.",
    });
