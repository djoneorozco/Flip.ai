// ============================================================
// ✅ Flip.ai Backend — Smart Budget + DALL·E CreateEdit (Middleman)
// ============================================================
...
// ============================================================
// ✅ /api/enhance — DALL·E CreateEdit + Tier Prompts + Fallback Mask
// ============================================================
...
Replace windows with clear modern windows, repaint exterior with fresh coat matching original color, add simple landscaping (fresh grass, trimmed bushes). Keep realistic, no major redesign.
    `.trim();
  } else {
    tier = "Tier 3 — Premium";
    stylePrompt = `
...
Keep realistic look for a premium flip.
  ...
  try {
    // ✅ Provide mask — for now, just use same file for image & mask as fallback
    const dalleResponse = await openai.images.createEdit({
      model: "dall-e-2",
      image: fs.createReadStream(imagePath),
      mask: fs.createReadStream(imagePath), // fallback, same image
      prompt: stylePrompt,
      n: 1,
      size: "1024x1024"
    });
    ...
    res.json({
      enhancedImageUrl,
      tierUsed: tier,
      description: "Generated by Flip.ai with real DALL·E createEdit & tiered prompt.",
    });
